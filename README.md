...
.
├── README.md          # 本说明
├── skills-zh/         # 中文技能（每个子目录一枚技能）
│   └── <skill-name>/
│       ├── SKILL.md
│       └── …          # 脚本、参考文档等（可选）
└── skills/            # 英文技能（每个子目录一枚技能）
    └── <skill-name>/
        ├── SKILL.md
        └── …
```

- **`skills-zh/`**：中文说明与指令为主，适合中文工作流与本土平台语境。  
- **`skills/`**：英文说明与指令为主，适合国际化协作或英文 Agent 环境。
...
- **中文技能**放在 `skills-zh/`，**英文技能**放在 `skills/`。  
- 同一主题的中英两版可共用相似目录名，但内容互不混放，避免单文件双语混杂导致 Agent 遵循不稳定。  
- 更新某技能时，请同时检查：frontmatter、`description`、正文中的链接与脚本路径是否仍有效。
...
1. Fork 本仓库  
2. 在 `skills-zh/` 或 `skills/` 下新增或修改对应子目录  
