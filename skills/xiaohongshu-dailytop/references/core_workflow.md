...
- **认证方式**：请求头 `X-API-KEY`，从环境变量 `REDFOX_API_KEY` 获取；脚本通过 `requests` 发起请求
...
- **输出顺序**：严格按照每日爆款笔记 → 每日分析 → HTML页面 → 订阅服务的顺序执行，禁止混乱输出
...
- HTML页面是4项输出中的第3项，必须在每日爆款笔记和每日分析之后输出
- 不需要等待用户回复，每日分析输出后立即执行
...
- 必须在HTML页面输出完成后展示
- 提供每日19:30推送小红书每日爆款笔记TOP50的订阅选项
- 用户可回复数字或"取消"
- **禁止遗漏，禁止胡乱输出**
...
```
🔔 订阅服务（是否需要订阅小红书每日爆款笔记推荐？）
1. 每日19:30-推送小红书每日爆款笔记TOP50
2. 暂不需要-仅本次查询

请回复数字或"取消"。
```

**订阅逻辑**：小红书每日爆款笔记推荐TOP50，按每日19:30更新

**输出顺序（严格按此顺序执行，禁止混乱输出，禁止遗漏）**：

1. 🔥 每日爆款笔记（含更新时间）
2. 📊 每日爆款笔记分析（3个模块：热门内容分析、爆款标题分析、最高数据作品分析）
...
- **预期产出**：匹配化妆美容分类，依次输出每日爆款笔记、每日分析、HTML页面、订阅服务
...
- 数据每日19:00更新昨天的榜单
- **输出内容（强制，禁止遗漏，禁止胡乱输出）**：必须严格包含每日爆款笔记（更新时间）+ 每日爆款笔记分析 + HTML页面 + 订阅服务
  - 🔥 每日爆款笔记（必须包含更新时间）
  - 📊 每日爆款笔记分析（3个模块：热门内容分析、爆款标题分析、最高数据作品分析）
  - 📁 HTML页面（直接输出HTML页面，禁止只输出文件路径）
  - **订阅服务（强制输出，禁止遗漏，禁止跳过，禁止等待用户回复）**：🔔 订阅服务（是否需要订阅小红书每日爆款笔记推荐？）
...
- **订阅服务输出要求（强制，禁止遗漏）**：
  ...
  - 提供订阅选项：每日19:30推送小红书每日爆款笔记TOP50 或 暂不需要-仅本次查询
  - 订阅逻辑：小红书每日爆款笔记推荐TOP50，按每日19:30更新
  - 请用户回复数字或"取消"
