...
- **总榜 / 分赛道榜单**：根据用户条件输出相应的爆款文章列表，支持多个分类供用户查询，支持全品类查询。
- **数据真实**：每日定时更新前一天的热门数据。
...
- **热门推荐**： 每日定时更新公众号10w+阅读文章，热门数据尽在掌握。
- **HTML 生成**： 将输出内容美化后展示，支持用户自行导出分享或保存。
- **订阅推送**：平台支持订阅的情况下支持订阅按时推送，掌握最新热门文章数据。

### 数据输出

- **脚本输出即正文**：表格 + 详情 + 统计区均不可删改重排。
- **更新时间**：早于 18:30 与晚于 18:30，「最新可用日期」不同，需配合固定致歉话术。
