...
- 实时热榜表格与HTML页面数据必须完全一致（相同排序、相同内容）
- 两个脚本使用相同的API接口和排序逻辑（互动数优先，备用计算：点赞+收藏*2+评论*3+分享\*5）
- 禁止胡乱渲染或使用不同数据源

**输出内容必须严格包含以下四部分，禁止遗漏或胡乱输出：**

- **实时热榜（更新时间）**：必须直接输出，禁止折叠
- **七日热门笔记分析**：必须直接输出，禁止折叠
- **HTML文件包**：必须最后生成并输出，禁止折叠
- **订阅服务（推送选项）**：必须在 HTML 文件包之后立即输出，禁止跳过
...
3. 📁 HTML文件包（实际文件，禁止只输出路径）
4. 订阅服务（推送选项，HTML 文件包输出完成后立即展示，禁止等待用户回复）
...
- 标题格式：**🔥 【{分类名称}】实时热榜 （更新时间：YYYY-MM-DD HH:MM:SS）**（标题整体加粗）
- 标题示例：**🔥 【化妆美容】实时热榜 （更新时间：2025-01-15 10:30:00）**
...
- **用户输入"查看更多"后**：
  - 实时热榜表格：输出TOP21到实际数据末尾（延续之前的TOP20）
...
- HTML文件包必须在所有内容输出完成后最后输出，不得提前或插入其他内容
- 不需要等待用户回复，七日分析输出后立即执行
- 这是输出的最后一步，必须在实时热榜、七日分析之后
- **禁止只输出文件路径，必须输出实际HTML文件包**
...
- HTML文件包必须在所有内容输出完成后最后输出，不得提前或插入其他内容
- 不需要等待用户回复，七日分析输出后立即执行
- 这是输出的第三步，必须在实时热榜、七日分析之后
- **禁止只输出文件路径，必须输出实际HTML文件包**
- **禁止折叠输出，禁止只输出文件路径，禁止遗漏**
- 这是唯一允许输出的文件，其他内容禁止输出为文件
...
### 第四部分：订阅服务（必须在HTML文件包之后）

**强制要求（必须严格遵守）**：

- 必须在HTML文件包输出完成后展示
- 提供每日19:30推送小红书七日热门笔记TOP50的订阅选项
- 用户可回复数字或"取消"
- **禁止遗漏，禁止胡乱输出**
...
**订阅逻辑**：小红书七日热门笔记推荐TOP50，按每日19:30更新
...
- **必须严格包含以上4项内容，禁止遗漏**
  ...
  - 📊 七日热门笔记分析（3个模块）
  - 📁 HTML文件包（实际文件）
  - **订阅服务（推送选项，必须在HTML文件包之后立即输出，禁止跳过，禁止遗漏）**
- **禁止胡乱输出，禁止输出其他内容**
- **禁止输出顺序混乱，必须严格按照上述顺序**
- **订阅服务必须强制执行，HTML文件包输出完成后必须立即展示订阅服务，不能等待用户回复，不能跳过，不能遗漏**
...
#### 第六步：生成HTML文件包（必须最后执行）

**重要要求：HTML文件包必须在所有内容输出完成后最后输出，不需要等待用户回复**
...
```
**📁 HTML文件包已生成** 路径：/path/to/xhs_weekly.html
```

#### 第七步：展示订阅服务（必须在HTML文件包之后强制执行）

**重要要求（强制，禁止跳过）**：必须在HTML文件包输出完成后立即展示订阅服务，不能等待用户回复，不能跳过，不能遗漏
...
**🔥 【综合全部】实时热榜** （更新时间：2026-04-15 19:30:00）
...
### 示例1：关键词查询冷门爆款（数据=50条）
- **场景/输入**："帮我查一下睫毛膏相关的爆款笔记"
- **执行流程**：
  1. 匹配分类：睫毛膏 → 化妆美容
  2. 查询化妆美容分类，返回50条数据
  3. 实时热榜表格展示TOP1-20（与HTML页面保持一致）
  4. 展示"回复"查看更多"可看剩余30条！✨"提示
  5. 输出完整分析（热门内容分析、爆款标题分析、最高数据作品分析）
  6. **最后生成HTML文件包（TOP1-20）**（与热榜表格数据完全一致）
  7. **展示订阅服务**（提供每日19:30推送选项）
...
- **执行流程**：
  1. 匹配对应分类
  2. 查询后返回15条数据
  3. 实时热榜表格展示TOP1-15（全部数据，与HTML页面保持一致）
  4. 不展示"查看更多"提示（数据未超过20条）
  5. 输出完整分析（热门内容分析、爆款标题分析、最高数据作品分析）
  6. **最后生成HTML文件包（TOP1-15）**（与热榜表格数据完全一致）
  7. **展示订阅服务**（提供每日19:30推送选项）

### 示例4：查看更多内容
- **场景/输入**：用户回复"查看更多"
- **执行流程**：
  1. 实时热榜表格展示TOP21-实际数据末尾（延续之前的数据）
  2. **最后重新生成HTML文件包（TOP1-实际数据末尾）**（完整数据，覆盖之前的文件）
...
## 注意事项
- 数据每日19:00更新昨天的榜单
- **输出内容（强制，禁止遗漏，禁止胡乱输出）**：必须严格包含实时热榜（更新时间）+ 七日热门笔记分析 + HTML文件包 + 订阅服务
  - 🔥 实时热榜（必须包含更新时间）
  - 📊 七日热门笔记分析（3个模块：热门内容分析、爆款标题分析、最高数据作品分析）
  - 📁 HTML文件包（必须输出实际文件，禁止只输出文件路径）
  - 订阅服务（提供每日19:30推送选项）
  - **禁止输出遗漏，禁止胡乱输出，必须严格按照此4项内容输出**
- **输出格式要求（强制）**：
  - **禁止折叠输出**：所有内容必须直接完整输出，禁止使用折叠、隐藏、展开等任何形式的折叠输出
  - **禁止输出文件**：除HTML文件包外，禁止输出任何其他文件（如CSV、JSON、TXT等）
  - **输出顺序**：严格按照实时热榜 → 七日分析 → HTML文件包 → 订阅服务的顺序执行，禁止混乱输出
- **输出顺序（严格按此顺序执行，禁止混乱输出）**：
  1. **🔥 实时热榜**（含更新时间，展示互动数/点赞/评论/收藏及新增数据，**不显示匹配到的分类名称**）
  2. **📊 七日热门笔记分析**（💡热门内容分析、📈爆款标题分析、✨最高数据作品分析）
  3. **📁 HTML文件包**（必须最后执行，七日分析后立即执行，不等待任何用户回复，必须输出实际文件）
  4. **订阅服务**（必须在HTML文件包之后展示，提供每日19:30推送小红书七日热门笔记TOP50的订阅选项）
- **HTML文件包输出要求（强制）**：
  - 必须在所有内容输出完成后最后输出，不得提前或插入其他内容之前
  - **禁止只输出文件路径，必须输出实际HTML文件包**
  - **禁止折叠输出**
  - **PDF和图片导出内容一定是正确的**
  - 不需要等待任何流程
- **订阅服务输出要求（强制，必须执行）**：
  - 必须在HTML文件包输出完成后立即展示
  - **禁止跳过，禁止遗漏，禁止等待用户回复**
  - 提供订阅选项：每日19:30推送小红书七日热门笔记TOP50 或 暂不需要-仅本次查询
  - 订阅逻辑：小红书七日热门笔记推荐TOP50，按每日19:30更新
  - 请用户回复数字或"取消"
- **数据一致性要求**：实时热榜表格与HTML页面必须使用相同的数据源和排序逻辑，确保完全一致
- **数据完整性要求（强制）**：
  - 表格所有字段和数据必须完整加载
  - 如果数据加载失败或字段缺失，必须重新调用脚本再次尝试
  - 直到所有字段都有完整数据（实时热榜7个字段、七日分析所有表格字段）
- **数据输出逻辑（必须严格执行）**：
  - 首次输出：实时热榜TOP1-20，HTML页面TOP1-20（保持一致）
  - 查看更多：实时热榜TOP21-实际数据末尾，HTML页面TOP1-实际数据末尾（完整数据）
  - **"查看更多"提示逻辑**：
    - 数据 ≤ 20条：不展示
