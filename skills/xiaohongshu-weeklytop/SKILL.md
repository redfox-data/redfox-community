...
- 提供实时热榜 + 智能分析 + 可导出 HTML 报告，支撑复盘与创作决策
...
- **⚡ 数据一致**：热榜表格与 HTML 页面使用相同 API 与排序逻辑，排名完全一致
...
> **用户**：帮我查一下睫毛膏相关的爆款笔记
>
> **助手**：自动匹配「化妆美容」分类 → 获取 TOP50 数据 → 输出实时热榜 TOP20 → 七日分析 → HTML 文件包 → 订阅选项

#### 2. 泛领域热门查询

> **用户**：最近小红书有什么热门内容
>
> **助手**：使用「综合全部」分类 → 按标准四步输出完整结果

#### 3. 查看更多数据

> **用户**：查看更多
>
> **助手**：延续输出 TOP21 至末尾 → 重新生成包含完整数据的 HTML 文件包
...
首次查询完成后，助手会展示订阅选项；回复 `1` 订阅每日 19:30 推送，回复 `2` 或「取消」跳过。

#### 4. 自定义 HTML 输出路径

```bash
python scripts/gen_xhs_html.py --keyword "穿搭" --top 20 --output ./reports/xhs_weekly.html
```
...
| `python scripts/gen_xhs_html.py --keyword "{词}" --top 20` | 首次生成 HTML（TOP20） |
| `python scripts/gen_xhs_html.py --keyword "{词}" --top 50` | 查看更多后生成完整 HTML |
| 用户说「查看更多」 | 输出 TOP21+ 并重新生成完整 HTML |
| 用户回复 `1` | 订阅每日 19:30 推送 |
| 用户回复 `2` / 「取消」 | 跳过订阅 |
...
| 报告生成 | `gen_xhs_html.py` | 渲染 HTML 页面，支持 PDF/图片导出 |
| 执行规范 | `core_workflow.md` | 定义输出格式、调用 Key、四步流程、订阅逻辑等强制约束 |
...
A: 两者须使用相同 API 与排序逻辑（互动数优先，备用：点赞+收藏×2+评论×3+分享×5）。检查是否使用了不同的 `--keyword` / `--category` 或 `--top` 参数。
...
- 四步输出顺序（热榜 → 分析 → HTML → 订阅）
- 禁止折叠输出、禁止输出除 HTML 外的文件
- 首次 TOP20 + 查看更多逻辑
- HTML 必须输出实际文件而非仅路径
- 订阅服务在 HTML 之后立即展示，禁止跳过
