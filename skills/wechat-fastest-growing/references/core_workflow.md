...
- **最新增长榜**：用户输入「今日增长榜」、「最新增长榜」、「公众号增长榜」
  → 调用脚本，传入 `rankDate: "yesterday"`（查询昨日榜单，昨日无数据自动查询前天）
...
- `source`：`"公众号阅读增长榜-GitHub"`

**调用示例**：

```bash
python scripts/fetch_growth_rank.py --rankDate "yesterday" --source "公众号阅读增长榜-GitHub"
python scripts/fetch_growth_rank.py --rankDate "2026-04-15" --source "公众号阅读增长榜-GitHub"
```
