...
[ ] 表格列顺序是否严格：排名→账号名称（含头像）→总粉丝数→涨粉率→粉丝增量
[ ] 排名1-3是否用了🥇🥈🥉，其余用数字
[ ] 账号名称是否包含头像+加粗+跳转链接（使用API返回的avatar和accountLink）
[ ] 总粉丝数、涨粉率、粉丝增量是否加粗
[ ] 数字>=1万是否用x.x万格式，>=1亿用x.x亿格式
[ ] TOP20后是否有"回复'查看更多'可看剩余30条！"提示
[ ] 分析部分是否是两段式：涨粉趋势分析→热点影响力量化（分析中账号名称仅纯文字，不含头像）
[ ] "以下内容分析基于获取的全部数据（TOP50）进行~"是否在分析标题后、趋势分析前
[ ] 功能服务提示是否在分析之后输出（仅首次查询时输出）
```
...
💡 榜单说明：抖音{日榜/周榜/月榜}{每日18:00/每周一18:00/每月1号18:00}更新{前一日/前一周/上一月}数据，本次排名数据的获取时间为{数据获取日期(如2026-04-27)}，与实时数据存在差异。

> 榜单说明文案根据榜单类型动态生成：日榜→"每日18:00更新前一日数据"；周榜→"每周一18:00更新前一周数据"；月榜→"每月1号18:00更新上一月数据"
...
- 平均涨粉率趋势：统计TOP50账号的平均涨粉率，如日榜平均涨粉率从XX%升至XX%，说明整体账号涨粉势头增强/减弱
- 粉丝增量分布：统计TOP50账号的粉丝增量区间分布（如10万以上X个、5-10万X个、1-5万X个），判断涨粉集中度
- 粉丝基数与增量的关系：分析大粉丝基数账号（>500万）与小粉丝基数账号的涨粉率差异，判断是"大号稳增"还是"小号爆发"

#### 热点影响力量化

- 特殊事件节点前后对比：如618大促期间消费类账号涨粉率集体上升XX%、政策出台后财经类账号入榜数增加XX个
- 热点对粉丝增长的拉动效果：量化特殊事件期间相关分类账号的平均粉丝增量增幅（如+XX%）、涨粉率提升幅度（如+XX个百分点）
- 行业涨粉周期规律：总结热点事件的涨粉拉动持续周期（如促销类约X天、政策类约X天），以及非热点期间的涨粉回落规律
...
**用户输入"查看更多"后**：
```bash
python scripts/gzh_growth_fetcher.py --rank_type day --keyword "科技" --top_n 30 --start 21
```
...
2. 输出第二部分：涨粉分析（两段式：涨粉趋势分析→热点影响力量化）
