...
API_HOST = "redfox.hk"
API_PATH = "/story/api/xhsData/query"


def _http_post(url: str, payload: dict, timeout: int = 15) -> dict:
    """
    使用 requests 发送 HTTPS 请求（正常方式，验证SSL证书）
    """
    headers = {
...
def _ssl_post_no_sni(path: str, payload: dict, host: str, timeout: int = 15) -> dict:
    """
    [已废弃] 使用原生 socket + ssl 发送 HTTPS 请求，禁用 SNI 绕过某些服务器限制
    """
...
def main():
    ...
    if args.html:
        ...
        html_path = Path("/workspace/projects") / html_filename
