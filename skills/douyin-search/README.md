...
- **关键词搜索**：输入内容主题关键词，返回抖音平台上与该主题相关的高互动爆款作品。
- **智能关键词泛化**：自动去除"今日""热门""爆款"等修饰词，提取核心主题，确保每次搜索都有结果。例如输入"今日美食热门"，会自动以"美食"进行查询。
- **结构化表格展示**：默认展示前 20 条作品，包含可点击跳转的作品标题、作者、点赞数、评论数、分享数、收藏数，超过 1 万的数字自动转为"x.xw"格式，一目了然。
- **查看全部数据**：当结果超过 20 条时，可一键展开查看最多 50 条完整数据。
- **关键词订阅推送**：查询完成后可订阅当前关键词，每天 10:00 自动查询最新爆款作品并推送通知。
...
| --- | ------------------------------- | --------- | ------ | ------ | ------ | ------ |
| 1   | [跟我一起来做这道菜…](链接)     | 美食达人A | 305.2w | 7.1w   | 51.0w  | 14.7w  |
| 2   | [旅行vlog：周末说走就走…](链接) | 旅行博主B | 158.3w | 3.2w   | 22.1w  | 8.5w   |
