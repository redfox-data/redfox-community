...
脚本内置公共 API Key，提供约 **10000 次免费额度**。直接运行命令即可开始生成视频。
...
公共 Key 额度用完后，前往 [redfox.hk](https://redfox.hk/settings/api-keys?source=github) 注册账号获取自己的 API Key，三种配置方式任选其一：
...
| 变量名 | 必填 | 说明 |
|--------|------|------|
| `REDFOX_API_KEY` | — | redfox.hk 平台的 API 访问密钥（首次使用无需配置） |
...
A：**无需配置，开箱即用。** 脚本内置公共 API Key，约 10000 次免费额度。额度用完后，前往 [redfox.hk](https://redfox.hk/settings/api-keys?source=github) 注册获取个人 Key，通过环境变量、命令行参数或配置文件配置即可。详见上方「后续使用」章节。
