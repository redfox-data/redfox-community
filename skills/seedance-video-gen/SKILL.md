...
seedance2.0开源版是一款基于火山方舟 **Seedance 2.0** 模型的 AI 视频生成工具。通过 redfox.hk 平台封装了复杂的 ARK API 鉴权流程，**一行命令即可生成视频**。
...
- Python 3.6+
- 一个 redfox.hk 账号及其 API Key（前往 [redfox.hk](https://redfox.hk/settings/api-keys?source=github) 注册获取）
...
```bash
python3 "$SKILL_PATH/scripts/videogen.py" "提示词" --api-key ak_你的密钥
```
...
| 变量名 | 必填 | 说明 |
|--------|------|------|
| `REDFOX_API_KEY` | 是 | redfox.hk 平台的 API 访问密钥 |
...
按照上方"一键安装"中的任意一种方式配置你的 API Key。
...
**Q1：提示"未配置 API Key"怎么办？**

A：你需要先前往 [redfox.hk](https://redfox.hk/settings/api-keys?source=github) 注册账号获取 API Key，然后通过环境变量、命令行参数或配置文件三种方式之一配置。详见上方「一键安装」章节。
