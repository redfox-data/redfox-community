...
"""
小红书评论分析脚本
调用 Redfox API 获取小红书笔记一级评论数据，同步生成 HTML 报告
用法: python3 xiaohongshu_comment_search.py "<noteId>" [--cursor ""] [--page 1] [--no-html] [--output-dir ~/Downloads/QoderReports]
"""
...
API_URL = "https://redfox.hk/story/api/xhs/ability/commentList"
...
def call_api(note_id: str, cursor: str, api_key: str) -> dict:
    """单次调用评论接口，返回原始 JSON 数据"""
    payload = json.dumps({
        "noteId": note_id,
        "cursor": cursor or "",
        "sort": "",
        "source": "小红书评论分析-GitHub",
    }).encode("utf-8")
...
def format_comment(raw: dict) -> dict:
    ...
    commentId, commentText, thumbCount, replyCount, postTime,
    ipRegion, userName, userAvatar, userUid, accountId
    """
    user_uid = raw.get("userUid", "") or ""
    nickname = (raw.get("userName", "") or "").strip()
    avatar = raw.get("userAvatar", "") or ""
    user_url = f"https://www.xiaohongshu.com/user/profile/{user_uid}" if user_uid else ""

    return {
        "comment_id": raw.get("commentId", ""),
        "content": (raw.get("commentText", "") or "").strip(),
        "like_count": raw.get("thumbCount", 0) or 0,
        "reply_count": raw.get("replyCount", 0) or 0,
        "create_time": raw.get("postTime", "") or "",
        "ip_location": raw.get("ipRegion", "") or "",
        "user_name": nickname,
        "user_avatar": avatar,
        "user_id": user_uid,
        "user_url": user_url,
        "is_top": bool(raw.get("isPinned", False) or raw.get("isTop", False)),
    }


def fetch_comments(note_id: str, cursor: str, api_key: str) -> tuple:
    """
    单次调用接口获取当前页评论数据（每次仅调用一次 API）。
    返回 (comments_list, has_next, next_cursor)
    """
    result = call_api(note_id, cursor, api_key)
    ...
    # 判断是否有下一页：基于 nextCursor 是否有值（hasMore 可能为 None）
...
def generate_html_report(note_id: str, current_page: int, comments: list, output_dir: str = None) -> str:
    """
    ...
    filename = f"小红书评论分析_{note_id}_p{current_page}.html"
...
def main():
    parser = argparse.ArgumentParser(description="小红书评论分析")
    parser.add_argument("note_id", help="小红书笔记 noteId")
    parser.add_argument(
    ...
    if not note_id:
        print("[error] noteId 不能为空", file=sys.stderr)
        sys.exit(1)

    api_key = get_api_key()

    comments, has_next, next_cursor = fetch_comments(note_id, args.cursor, api_key)
