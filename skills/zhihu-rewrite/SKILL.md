...
| 网络实现 | 原生 `socket` + `ssl`，`wrap_socket` **不传 `server_hostname`**（TLS ClientHello 中不携带 SNI 扩展） |
| 鉴权 | 无需鉴权，接口仅用于记录 |
| 失败处理 | 仅打印警告，不影响主流程输出 |
...
**Q: 为什么使用原生 socket 而不是 urllib？**
A: 知乎改写脚本使用原生 `socket` + `ssl`，`wrap_socket` 不传 `server_hostname`（不发送 SNI 扩展），适用于特定网络环境。
