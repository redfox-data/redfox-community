---
name: douyin-comment
description: 抖音作品评论查询工具。根据作品 videoId 获取一级评论数据，支持分页浏览、评论情感分析（积极/负面/需求/竞品），生成精美 HTML 报告。当用户需要查看抖音作品评论、分析评论舆情、了解用户反馈时使用。触发词：抖音评论、作品评论、评论查询、评论分析、评论舆情、看评论。
---
...
根据抖音作品 videoId 获取一级评论数据，每页 20 条，支持分页翻页。在对话中展示当前页全部评论及 AI 总结分析，同时生成包含当前页数据和总结的交互式 HTML 报告。

## ✨ 功能特性

| 功能模块 | 能力描述 | 核心价值 |
|---------|---------|---------|
| 评论获取 | 根据 videoId 获取一级评论 | 实时拉取作品评论区数据 |
...
- 若用户未提供 videoId，主动询问：「请提供抖音作品的 videoId 或作品链接」
...
| videoId 无效 | 提示「未找到该作品的评论，请检查 videoId 是否正确」 |
| 接口返回错误 | 显示错误码和错误信息 |
